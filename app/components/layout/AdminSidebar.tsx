...
const AdminSidebar = ({ isMobileOpen = false, onMobileClose }: AdminSidebarProps) => {
  ...
  const { pendingTotalCount: pendingTransactions, isLoading: transactionsLoading } = useAdminTransactions()
  ...
  const navigationItems = [
    ...
    },
    {
      ...
      badge: transactionsLoading ? '...' : (pendingTransactions > 0 ? pendingTransactions : undefined)
    },
    {
            ...
            {navigationItems.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`group flex items-center justify-between px-3 py-2 text-sm font-medium rounded-lg transition-all duration-200 ${item.current
                      ? `bg-gradient-to-r from-blue-50 to-indigo-50 text-blue-700 ${locale === 'ar' ? 'border-r-4 border-blue-500' : 'border-l-4 border-blue-500'
                      }`
                ...
                >
                  <div className="flex items-center">
                    <Icon
                      className={`w-5 h-5 ${locale === 'ar' ? 'ml-3' : 'mr-3'} ${item.current ? 'text-blue-600' : 'text-gray-400 group-hover:text-gray-500'
                        }`}
                    />
                    {item.name}
                  </div>
                  {item.badge && (
                    <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${item.href === '/admin/deposits'
