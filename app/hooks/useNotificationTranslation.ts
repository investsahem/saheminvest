...
export function useNotificationTranslation() {
  ...
  const translateNotification = (text: string, metadata?: any) => {
    ...
    // If text looks like a translation key (e.g., "notifications.something"), translate it
    if (text && text.startsWith('notifications.')) {
      // Build variables from metadata for interpolation
      const variables: Record<string, any> = {}

      // Try to get variables from i18n metadata first
      if (parsedMetadata?.i18n?.variables) {
        Object.assign(variables, parsedMetadata.i18n.variables)
      }

      // Also extract common fields from metadata directly
      if (parsedMetadata) {
        if (parsedMetadata.amount) variables.amount = parsedMetadata.amount.toLocaleString?.() || parsedMetadata.amount
        if (parsedMetadata.dealTitle) variables.dealTitle = parsedMetadata.dealTitle
        if (parsedMetadata.partnerName) variables.partnerName = parsedMetadata.partnerName
        if (parsedMetadata.investorName) variables.investorName = parsedMetadata.investorName
        if (parsedMetadata.userEmail) variables.userEmail = parsedMetadata.userEmail
        if (parsedMetadata.reference) variables.reference = parsedMetadata.reference
      }

      // Handle payment method translation
      if (variables.method) {
        if (variables.method === 'cash') {
          variables.method = t('notifications.payment_method_cash')
        } else if (variables.method === 'bank transfer') {
          variables.method = t('notifications.payment_method_bank')
        }
      }
