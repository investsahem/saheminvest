    ...
    <Card className="relative overflow-hidden hover:shadow-lg transition-all duration-300 border-0 shadow-sm">
      <CardContent className="p-6">
        <div className="flex items-center justify-between">
          ...
          <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${trend === 'up' ? 'bg-green-50' : 'bg-red-50'
            }`}>
            <Icon className={`w-6 h-6 ${trend === 'up' ? 'text-green-600' : 'text-red-600'}`} />
          </div>
        </div>
        <div className={`absolute bottom-0 left-0 h-1 w-full ${trend === 'up' ? 'bg-gradient-to-r from-green-400 to-green-600' :
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="bg-green-50 p-4 rounded-lg">
                  <div className="text-2xl font-bold text-green-600">{dashboardData.pendingApprovals}</div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-medium text-blue-600">{formatCurrency(dashboardData.monthlyRevenue)}</span>
                </div>
              </div>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-purple-50 p-4 rounded-lg">
                  <div className="text-2xl font-bold text-purple-600">{formatNumber(dashboardData.totalUsers)}</div>
                  <div className="text-sm text-purple-800">{t('admin_dashboard.total_investors')}</div>
                </div>
                <div className="bg-indigo-50 p-4 rounded-lg">
                  <div className="text-2xl font-bold text-indigo-600">{formatCurrency(dashboardData.totalRevenue)}</div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  ...
                  <span className="text-sm font-medium text-green-600">{dashboardData.activeDeals}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">{t('admin_dashboard.avg_Portfolio_size')}</span>
                  <span className="text-sm font-medium text-blue-600">{dashboardData.totalUsers > 0 ? formatCurrency(dashboardData.totalRevenue / dashboardData.totalUsers) : '$0'}</span>
                </div>
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">{t('admin_dashboard.portfolio_growth')}</span>
                  <span className="text-sm font-medium text-green-600">{dashboardData.revenueGrowth >= 0 ? '+' : ''}{dashboardData.revenueGrowth.toFixed(1)}%</span>
                </div>
              </div>
          ...
          <CardContent className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="text-center">
                <div className="text-3xl font-bold text-gray-900 mb-1">{formatCurrency(dashboardData.totalRevenue)}</div>
                <div className="text-sm text-gray-600 mb-2">{t('admin_dashboard.total_invested')}</div>
                <div className="flex items-center justify-center text-green-600">
                  <TrendingUp className="w-4 h-4 mr-1" />
                  <span className="text-sm font-medium">{dashboardData.revenueGrowth >= 0 ? '+' : ''}{dashboardData.revenueGrowth.toFixed(1)}%</span>
                </div>
              </div>

              <div className="text-center">
                <div className="text-3xl font-bold text-gray-900 mb-1">{formatCurrency(dashboardData.totalProfit)}</div>
                <div className="text-sm text-gray-600 mb-2">{t('admin_dashboard.total_returns')}</div>
                <div className="flex items-center justify-center text-green-600">
                  <TrendingUp className="w-4 h-4 mr-1" />
                  <span className="text-sm font-medium">{dashboardData.avgReturn >= 0 ? '+' : ''}{dashboardData.avgReturn.toFixed(1)}%</span>
                </div>
              </div>

              <div className="text-center">
                <div className="text-3xl font-bold text-gray-900 mb-1">{dashboardData.avgReturn.toFixed(1)}%</div>
                <div className="text-sm text-gray-600 mb-2">{t('admin_dashboard.avg_roi')}</div>
                <div className="flex items-center justify-center text-green-600">
                  <TrendingUp className="w-4 h-4 mr-1" />
                  <span className="text-sm font-medium">{dashboardData.avgReturnChange >= 0 ? '+' : ''}{dashboardData.avgReturnChange.toFixed(1)}%</span>
                </div>
              </div>

              <div className="text-center">
                <div className="text-3xl font-bold text-gray-900 mb-1">{dashboardData.successRate.toFixed(1)}%</div>
