...
interface PendingDistribution {
  ...
  estimatedInvestorCapitalReturn: number
  estimatedInvestorTotal: number
}
...
export default function InvestorDashboard() {
  ...
  const { portfolio, dailyChange, investments, pendingDistributions, summary } = portfolioData
    ...
    <InvestorLayout title={t('investor.portfolio_dashboard')} subtitle={t('investor.track_investment_performance')}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          {/* Pending Estimated Profits */}
          {(pendingDistributions && pendingDistributions.length > 0) && (
            <Card className="bg-gradient-to-r from-amber-50 to-orange-50 border-amber-200">
              <CardContent className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <p className="text-sm font-medium text-amber-700">{t('investor.pending_estimated_profit') || 'الأرباح المتوقعة'}</p>
                    <p className="text-2xl font-bold text-amber-900">
                      +{formatCurrency(portfolio.pendingEstimatedProfit || 0)}
                    </p>
                  </div>
                ...
                <div className="text-sm text-amber-700">
                  {t('investor.pending_distributions_count')?.replace('{count}', pendingDistributions.length.toString()) ||
                    `${pendingDistributions.length} توزيعات قيد التنفيذ`}
                </div>
              </CardContent>
            </Card>
          )}
